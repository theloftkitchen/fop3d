...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext) 
{
	...
	if(m_Splitter.CreateStatic(this,1,2)== NULL)
		return FALSE;
	if(!m_Splitter.CreateView(0,0,RUNTIME_CLASS(CDemoView),CSize(cx*3/4,cy),pContext))
	{
		m_Splitter.DestroyWindow();
		return FALSE;
	}
	if(!m_Splitter.CreateView(0,1,RUNTIME_CLASS(CControlWnd),CSize(cx/4,cy),pContext))
	{
		m_Splitter.DestroyWindow();
		return FALSE;
	}
